...
void main() {
    ...
    mediump vec2 tex_offset = vec2(1.0 / float(tex_size.x), 1.0 / float(tex_size.y)); // gets size of single texel

    mediump vec3 result = texture(texture0, fragTexCoord).rgb * weight[0]; // current fragment's contribution
