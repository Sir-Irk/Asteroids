...
main(void)
{
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_SPACE)) {
            ...
            push_bullet(&bullet_array, (bullet_t){player.position, player.position, Vector2Scale(direction, 800.0f), GetTime()});
        }
        ...
        for (i32 i = 0; i < bullet_array.count; ++i) {
            bullet_t *b = &bullet_array.bullets[i];
            DrawCircle(b->position.x, b->position.y, 16.0f, BLUE);
        }
